...
-- [ 쇼핑몰 조건 ]
-- 1. 회원 정보를 저장하고,  - 회원 정보 테이블
-- 	회원번호, ID, 이름, 주소
-- 2. 주문리스트를 저장하고 싶어요. - 주문리스트 테이블
-- 	회원번호, 주문번호, 상품번호, 수량, 결제 금액
-- 3. 상품리스트 테이블
-- 	상품번호, 상품명, 상품 가격
-- 4. 포인트 테이블
-- 	회원번호, 포인트
...
-- 삭제해도 데이터가 필요하다고 생각하면 NO ACTION
); 
...
-- mem_no를 안 넣은 이유는 AUTO_INCREMENT가 설정되어 있으므로

INSERT INTO points(mem_no)
VALUES(1);
-- pt를 안 넣은 이유는 디폴트값(0)이 설정되어 있으므로
...
-- 	** 늘리는 건 늘리되 웬만해선 줄이기는 X, 에러남

-- 	- 컬럼 삭제
-- 		ALTER TABLE 테이블명 DROP COLUMN 컬럼;
ALTER TABLE members DROP COLUMN age;

-- ** ALTER TABLE로 외래키 넣는 방법
-- ALTER TABLE orders ADD CONSTRAINT fk_orders_mem_no FOREIGN KEY(mem_no)
-- 		REFERENCES members(mem_no) ON DELETE CASCADE 
-- ALTER TABLE orders ADD CONSTRAINT fk_orders_product_no FOREIGN KEY(product_no)
-- 		REFERENCES products(product_no) ON DELETE NO ACTION
...
-- ** TRUNCATE는 ROLLBACK 안 먹힘, 현업에서 잘 사용 안함
