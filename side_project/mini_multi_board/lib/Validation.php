...
// 맨 앞 대문자
class Validation {
    private static $arrErrorMsg = []; // 발리데이전용 에러메시지 저장 프로퍼티
    ...
    // 유효성 체크 메소드
    public static function userChk(array $inputData) : bool { // 정규식 이용
        ...
        if(array_key_exists("u_pw", $inputData)) {
            if(preg_match($patternPw, $inputData["u_pw"], $match) === 0) {
                // PW 에러처리
                $msg = "비밀번호는 영어 대소문자와 숫자, !, @로 8~20자로 입력해 주세요.";
                self::setArrErrorMsg($msg);
            }
        }
        // 비밀번호 확인 체크
        ...
        if(array_key_exists("u_name", $inputData)) {
            if(preg_match($patternName, $inputData["u_name"], $match) === 0) {
                // NAME 에러처리
                $msg = "이름은 영어 대소문자와 한글로 2~50자로 입력해 주세요.";
                self::setArrErrorMsg($msg);
            }
        }
        // 리턴 처리
